...
INSERT INTO `Food` VALUES (1,'snack',6.00),(2,'meal',7.00),(3,'buffet',8.00),(4,'drink',5.00);
...
CREATE TABLE `orders` (
  ...
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`item_id`) REFERENCES `Food` (`item_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
BEGIN
    ...
    -- Check if the item_name exists in the Food table
    IF (SELECT COUNT(*) FROM Food WHERE name = p_item_name) > 0 THEN
...
BEGIN
    ...
    SET v_item_id = (SELECT item_id FROM Food WHERE name = p_food_item);
